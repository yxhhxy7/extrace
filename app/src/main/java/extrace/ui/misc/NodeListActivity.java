...
import extrace.loader.NodeListLoader;
...
public class NodeListActivity extends AppCompatActivity {
	...
	public static class PlaceholderFragment extends ListFragment {
		...
		private NodeListLoader mLoader;
		...
		@Override public void onActivityCreated(Bundle savedInstanceState) {
            ...
            mAdapter = new NodeListAdapter(new ArrayList<TransNode>(), this.getActivity());
    		...
    		if (mIntent.hasExtra("Action")) {
    			if(mIntent.getStringExtra("Action").equals("New")){
    				...
    				if (mIntent.hasExtra("CustomerInfo")) {
    					selectItem = (TransNode) mIntent.getSerializableExtra("CustomerInfo");
        ...
        public void onActivityResult(int requestCode, int resultCode, Intent data) {	//编辑界面的回调
    		...
    		case RESULT_OK:
    			...
    			if (data.hasExtra("CustomerInfo")) {
    				customer = (TransNode) data.getSerializableExtra("CustomerInfo");
...
//    			}
		...
		{
	        mIntent.putExtra("NodeInfo",selectItem);
		...
		private void RefreshList(String name)
		{
			this.getActivity().setTitle("");
				...
				try {
					mLoader = new NodeListLoader(mAdapter, this.getActivity());
					mLoader.LoadTransNodeListByName(name);
				} catch (Exception e) {
					e.printStackTrace();
				}
